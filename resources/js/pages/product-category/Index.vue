<template>
	...
	>
		<div class="flex py-2 px-4 bg-accent items-center justify-between gap-2">
			<div class="relative">
				<Form :action="route('admin.categories.index', {type: EnumCategoryType.PRODUCT})" method="get">
					<Input class="bg-white" type="text" placeholder="Tìm kiếm" name="search" />
					<Input type="hidden" name="type" :value="EnumCategoryType.PRODUCT" />
			...
			<div class="flex gap-2">
				<Link
					:href="route('admin.categories.create', {type: EnumCategoryType.PRODUCT})"
		...
		<DataTable
			...
			:deleteUrl="route('admin.categories.destroy.many', {type: EnumCategoryType.PRODUCT})"
		></DataTable>
	</AppLayout>
</template>
...
<script lang="ts" setup>
	...
	const { paginationData } = usePagination('admin.categories.index', {type: EnumCategoryType.PRODUCT})
